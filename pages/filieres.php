...
if($niveau=="all")
    {
        $requete="select * from filiere
                    where nomFiliere like '%$nomf%' limit  $size
                    offset $offset";
    ...
    }else
        {
            $requete="select * from filiere where nomFiliere like '%$nomf%'
             and niveau='$niveau' limit $size offset $offset ";
    ...
    $reste = $nbrFiliere % $size; // % operateur module: le reste de la division
        ...
        <div class="panel panel-success margetop60">
            ...
            <form method="get" action="filieres.php" class="form-inline">
                ...
                <select name="niveau" class="form-control" id="niveau" onchange="this.form.submit()">
                    ...
                    <option value="q" <?php if($niveau==="all") echo "selected" ?>> Qualification</option>
                    <option value="t" <?php if($niveau==="all") echo "selected" ?>> Technicien</option>
                    <option value="ts" <?php if($niveau==="all") echo "selected" ?>> Technicien spécialisé</option>
                    <option value="l" <?php if($niveau==="all") echo "selected" ?>> Licence</option>
                    <option value="m" <?php if($niveau==="all") echo "selected" ?>> Master</option>
                </select>
                            ...
                            <?php while($filiere=$resultatF->fetch()){ ?>
                                <tr>
                                    ...
                                    <td> <?php echo $filiere['niveau']?></td>
                                     <?php if ($_SESSION['user']['role']== 'ADMIN') { ?>
                                    <td>
                                            <?php  $idF= isset($_GET['idFiliere'])?$_GET['idFiliere']:0; ?>
